...
/*
https://github.com/Azure/azure-sdk-for-c-arduino/blob/main/examples/Azure_IoT_Hub_ESP8266/readme.md
...
// Wifi
//#define IOT_CONFIG_WIFI_SSID "CEO Home" 
//#define IOT_CONFIG_WIFI_PASSWORD "Graphics$1"

//Alt wifi
#define IOT_CONFIG_WIFI_SSID "LAPTOP-HJ2RMVC9 0795" 
#define IOT_CONFIG_WIFI_PASSWORD "9272y6R)"

// Azure IoT
#define IOT_CONFIG_IOTHUB_FQDN "2025TechathonIoTHub.azure-devices.net"
#define IOT_CONFIG_DEVICE_ID "ESP8266-TZO2025"
#define IOT_CONFIG_DEVICE_KEY "ExLwEC3Zvh4by8NUdzwXV9iRxV4os/vs/7ewawu1w3k="

// Publish 1 message every 1 min
#define TELEMETRY_FREQUENCY_MILLISECS 60000

#define TELEMETRY_AUTO_STOP 120000 // 2 min
...
#define LED_PIN 2 // ESP Internal LED
...
static uint8_t panic_level = 0;
static uint8_t gpt_response = 0;
static uint8_t button_press = 0;
static unsigned long telemetry_start_time_ms = 0;
...
const int low_button_pin = 2; // 0 button
int low_button_state = 0; 

const int high_button_pin = 3; // 0 button
int high_button_state = 0; 

const int fart_button_pin = 13; // 0 button
int fart_button_state = 0; 
int GPSflag = 0;

// Auxiliary functions
IPAddress Mobile(198,168,196,138);
...
static char* getTelemetryPayload()
{
  ...
  //Time
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR("{ \"Local Date\": "));
  (void)az_span_u32toa(temp_span, getCurrentLocalTime()->tm_mon, &temp_span);
  (void)az_span_u32toa(temp_span, getCurrentLocalTime()->tm_mday, &temp_span);
  (void)az_span_u32toa(temp_span, getCurrentLocalTime()->tm_year, &temp_span);

  //Panic level
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(", \n \"Panic Level\": "));
  (void)az_span_u32toa(temp_span, panic_level, &temp_span); 

  //GPT on/off
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(", \n \"GPT Response\": "));
  (void)az_span_u32toa(temp_span, gpt_response, &temp_span); 

  //GPS lat,long
 
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(", \n \"GPS Latitude\": "));
  (void)az_span_u32toa(temp_span,  returnLat(gps), &temp_span); 
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(", \n \"GPS Longitude\": "));
  (void)az_span_u32toa(temp_span,  returnLong(gps), &temp_span); 

  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(" }"));
  temp_span = az_span_copy_u8(temp_span, '\0');

  Serial.print(" Description: Message Count ");
...
void setup()
{
  ...
  pinMode(low_button_pin, INPUT);
  pinMode(high_button_pin, INPUT);
  pinMode(fart_button_pin, INPUT);
  digitalWrite(LED_PIN, HIGH);
     ss.begin(GPSBaud);
  establishConnection();
}
...
void loop()
{
    ...
    low_button_state = digitalRead(low_button_pin);
    high_button_state = digitalRead(high_button_pin);
    fart_button_state = digitalRead(fart_button_pin);

    if(low_button_state == HIGH){
        button_press = 1;
        telemetry_start_time_ms = millis();
        gpt_response = 1;
        panic_level = 0;
    } else if (high_button_state == HIGH){
        button_press = 1;
        telemetry_start_time_ms = millis();
        gpt_response = 1;
        panic_level = 1;
    } else if (fart_button_state == HIGH){
        button_press = 1;
        telemetry_start_time_ms = millis();
        gpt_response = 1;
        panic_level = 2;
    } else if (millis() > telemetry_start_time_ms + TELEMETRY_AUTO_STOP){
      button_press = 0;
      telemetry_start_time_ms = 0;
    } 
